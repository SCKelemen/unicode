...
func TestAnalyzeOverrideFailures(t *testing.T) {
	...
	for scanner.Scan() {
		...
		for paraLevel := 0; paraLevel <= 1; paraLevel++ {
			...
			if !levelsMatch || !reorderMatch {
				...
				// Check if this involves LRO or RLO
				hasLRO := false
				hasRLO := false
				...
				if hasLRO {
					lroFailures++
					if lroFailures <= 5 {
						t.Logf("LRO failure at line %d: %s (para=%d)", lineNum, classesToString(classes), paraLevel)
				...
				if hasRLO {
					rloFailures++
					if rloFailures <= 5 {
						t.Logf("RLO failure at line %d: %s (para=%d)", lineNum, classesToString(classes), paraLevel)
						t.Logf("  Expected levels: %v", expectedLevels)
						t.Logf("  Actual levels:   %v", actualLevels)
						t.Logf("  Expected reorder: %v", expectedReorder)
						t.Logf("  Actual reorder:   %v", actualReorder)
	...
	t.Logf("\n=== OVERRIDE FAILURE ANALYSIS ===")
	t.Logf("Total failures: %d", totalFailures)
	t.Logf("LRO failures: %d (%.1f%%)", lroFailures, float64(lroFailures)/float64(totalFailures)*100)
	t.Logf("RLO failures: %d (%.1f%%)", rloFailures, float64(totalFailures)*100)
	t.Logf("Other failures: %d", totalFailures-lroFailures-rloFailures)
}
